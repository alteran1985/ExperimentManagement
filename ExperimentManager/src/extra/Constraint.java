...
public class Constraint implements PropertyConstants {
	...
	public Collection<Feature> getDeadFeatures(SatSolver solver, FeatureModel fm, Collection<Feature> fmDeadFeatures) {
		...
		if (propNode != null) {
			deadFeaturesBefore = fm.getAnalyzer().getDeadFeatures(solver, propNode);
		} else {
	...
	public Collection<Feature> getDeadFeatures(FeatureModel fm, Collection<Feature> fmDeadFeatures) {
		...
		if (propNode != null) {
			...
			deadFeaturesBefore = fm.getAnalyzer().getDeadFeatures();
	...
	public boolean setFalseOptionalFeatures(FeatureModel clone,
			Collection<Feature> fmFalseOptionals) {
		return false;
	}
