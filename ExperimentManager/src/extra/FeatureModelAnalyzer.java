...
public class FeatureModelAnalyzer {
	...
	public FeatureModelAnalyzer(FeatureModel fm) {
		this.fm = fm;
	}
	...
	public boolean checkIfFeatureCombinationNotPossible(Feature a, Collection<Feature> b) throws Exception {
		if (b.isEmpty())
			return true;
	...
	public boolean areMutualExclusive(Collection<Feature> context,
			Collection<Set<Feature>> featureSets) throws Exception {
	 ...
	 * @throws TimeoutException
	 */
	public boolean exists(Collection<Feature> features) throws Exception {
		if ((features == null) || (features.isEmpty()))
			return true;
		...
		try {
			if (!cachedValidity) { 
				...
				for (Constraint constraint : fm.getConstraints()) {
					...
					try {
						if (!contraintFound && !clone.getAnalyzer().isValid()) {
